# Docker image with Olog service.
# Controls Group - Brazilian Synchrotron Light Laboratory (LNLS)
...
ARG OLOG_WEB_REPO
RUN set -x; set -e;\
    git clone ${OLOG_WEB_REPO} ${GLASSFISH_CONF_FOLDER}/logbook &&\
    cd ${GLASSFISH_CONF_FOLDER}/logbook &&\
    git checkout ${OLOG_WEB_HASH}
